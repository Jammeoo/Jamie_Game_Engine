...
void AudioSystem::UpdateListener(int listenerIndex, const Vec3& listenerPosition, const Vec3& listenerForward, const Vec3& listenerUp)
{
	...
	fmodSoundPosition.x = -listenerPosition.y;
	fmodSoundPosition.y = listenerPosition.z;
	fmodSoundPosition.z = -listenerPosition.x;
	...
	FMOD_VECTOR fmodForwardDir;
	fmodForwardDir.x = -listenerForward.y;
	fmodForwardDir.y = listenerForward.z;
	fmodForwardDir.z = -listenerForward.x;

	FMOD_VECTOR fmodUpDir;
	fmodUpDir.x = -listenerUp.y;
	fmodUpDir.y = listenerUp.z;
	fmodUpDir.z = -listenerUp.x;

	FMOD_RESULT result = m_fmodSystem->set3DListenerAttributes(listenerIndex, &fmodSoundPosition, &vel, &fmodForwardDir, &fmodUpDir);
	ValidateResult(result);
}
...
SoundPlaybackID AudioSystem::StartSoundAt(SoundID soundID, const Vec3& soundPosition, bool isLooped /*= false*/, float volume /*= 1.0f*/, float balance /*= 0.0f*/, float speed /*= 1.0f*/, bool isPaused /*= false*/)
{
	...
	if (channelAssignedToSound)
	{
		...
		channelAssignedToSound->setLoopCount(loopCount);
		FMOD_VECTOR fmodSoundPosition;
		fmodSoundPosition.x = soundPosition.x;
		fmodSoundPosition.y = soundPosition.y;
		fmodSoundPosition.z = soundPosition.z;
		FMOD_VECTOR vel;
		vel.x = 0.f;
		vel.y = 0.f;
		vel.z = 0.f;
		channelAssignedToSound->set3DAttributes(&fmodSoundPosition, &vel);
	}
	m_fmodSystem->playSound(sound, nullptr, isPaused, &channelAssignedToSound);
	return (SoundPlaybackID)channelAssignedToSound;
}
...
void AudioSystem::SetSoundPosition(SoundPlaybackID soundPlaybackID, const Vec3& soundPosition)
{
	...
	channelAssignedToSound->isPlaying(&soundIsPlaying);
	...
	vel.y = 0.f;
	vel.z = 0.f;
	FMOD_RESULT result = channelAssignedToSound->set3DAttributes(&fmodSoundPosition, &vel);
	ValidateResult(result);
}
...
bool AudioSystem::IsPlaying(SoundPlaybackID soundPlaybackID)
{
	FMOD::Channel* channelAssignedToSound = (FMOD::Channel*)soundPlaybackID;
	bool result = false;
	channelAssignedToSound->isPlaying(&result);
	return result;
}
